...
def preparation(segmentation_model, label_model, imagepath, model_filepath):
    ...
    test_img_area = normalize(gray_image, axis=1)
    # print("Shape image_data:", test_img.shape)  # Contoh: (N, SIZE, SIZE, 1)
    test_input_area = np.expand_dims(test_img_area,0)
    test_input_area = np.expand_dims(test_input_area,3)
    
    if ("singleUnet" in model_filepath):
        gray_image = normalize(gray_image, axis=1) # ganti kalau ada usage gray_image di bawahnya
    test_input_label = np.expand_dims(gray_image,0)

    # Call helper function untuk membangun fitur
    output_image, areas_label, area_lung, label_location = build_feature(test_input_area, test_input_label, segmentation_model, label_model, colors)
